...
public class GraphTask extends Task<Map<String, List<GraphLogEntry>>, Void> {
  private static ServiceLoader<GraphView> graphViewLoader = ServiceLoader.load(GraphView.class);
   ...
   *
   */
  public static boolean isGraphViewAvailable() {
    return graphViewLoader.iterator().hasNext();
  }
  ...
  protected void succeeded(Map<String, List<GraphLogEntry>> result) {
    ...
    for(GraphView graphView : graphViewLoader) {
      graphView.createGraph(row, location, result, branches);
