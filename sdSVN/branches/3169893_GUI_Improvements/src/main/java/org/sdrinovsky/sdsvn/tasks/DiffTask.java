...
public class DiffTask extends Task<Void, Void> {
  ...
  protected Void doInBackground() throws Exception {
    ...
    if(diffProgram != null) {
      if(revision1 != null) {
        diffWithRepo(diffProgram);
      } else if(diffTogether == false) {
        diffWithTextBase(diffProgram, mergeProgram);
        diffProperties(diffProgram);
      } else {
        diffTogether(diffProgram);
      }
    } else {
  ...
  private void diffTogether(String diffProgram) throws IOException {
    if(rows.size() == 2) {
      System.out.println("diffing " + rows.get(0).getFile() + " with " + rows.get(1).getFile());
      processDiff(Arrays.asList(new String[]{diffProgram, rows.get(0).getFile().getPath(),
                                             rows.get(1).getFile().getPath()}));
    }
  }
  ...
  private void processDiff(List<String> commangLine) throws IOException {
    ...
    new Thread(new Logger(process.getInputStream())).start();
    new Thread(new Logger(process.getErrorStream())).start();
  }
