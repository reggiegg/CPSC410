...
import org.sdrinovsky.sdsvn.diff.JMeldDiffView;
...
public class DiffTask extends Task<Void, Void> {
  ...
  protected Void doInBackground() throws Exception {
    boolean internalDiff = Boolean.parseBoolean(app.getPreferences().get("internalDiff", null));
  ...
  private void processInternalDiff(File leftFile, File rightFile, boolean leftReadOnly, boolean rightReadOnly) {
    JMeldDiffView diffView = new JMeldDiffView();

    try {
      diffView.executeDiff(leftFile.getPath(), rightFile.getPath(), leftReadOnly, rightReadOnly);
    }catch(Exception e) {
      e.printStackTrace();    //TODO: How should we be handling exceptions?
    }
  }
