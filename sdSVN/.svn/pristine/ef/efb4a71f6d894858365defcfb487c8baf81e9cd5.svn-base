...
public class MergeTask extends Task<Void, Void> {
  ...
  protected Void doInBackground() throws Exception {
    ...
    for(FileRow row : rows) {
      ...
      doMerge(destination, "", reintegrate);
    }
  ...
  private List<SVNRevisionRange> getEligible(FileRow row) throws SVNException {
    ...
    HistoryDialog dialog = new HistoryDialog(app.getMainFrame(), true, true);
    ...
    reintegrate = dialog.isReintegrate();
  ...
  private void doMerge(File destination, String path, boolean reintegrate) throws Exception {
    if(start == null) {
      ...
      if(reintegrate) {
        diffClient.doMergeReIntegrate(url, SVNRevision.HEAD, file, false);
      } else {
